...
namespace DBUtility
{
	...
	public abstract class DbHelperOra
	{
        ...
        public static string connectionString = System.Configuration.ConfigurationManager.ConnectionStrings["DbConnectionStringForMETAL"].ToString();
		public DbHelperOra()
		{			
		}
        ...
        public static int GetMaxID(string FieldName, string TableName, string strWhere)
        {
            ...
            {
                strSql.Append("select max(" + FieldName + ") + 1 from " + TableName);
            }
            if (strWhere.Trim()!="")
            {
                strSql.Append(" where DEL_FLAG = '00' " + strWhere);
            }
        ...
        /// <summary>
        /// 执行多条SQL语句，实现数据库事务。对应多条语句相同的情况
        /// </summary>
        /// <param name="SQLStringList">SQL语句的List</param>
        /// <param name="SQLStringList">OracleParameter[]的List</param>
        public static void ExecuteSqlTran(List<string> SQLStringList, List<OracleParameter[]> SQLParameterList)
        {
            using (OracleConnection conn = new OracleConnection(connectionString))
            {
                conn.Open();
                using (OracleTransaction trans = conn.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        for (int i = 0; i < SQLStringList.Count; i++)
                        {
                            ...
                            OracleParameter[] cmdParms = SQLParameterList[i];
                            PrepareCommand(cmd, conn, trans, cmdText, cmdParms);
                            int val = cmd.ExecuteNonQuery();
                            cmd.Parameters.Clear();
